...
function renderSettings(container) {
  container.innerHTML = `
    <div class="sf-connection-card">
      <div class="sf-conn-status ${SFStore.connected ? 'connected' : 'disconnected'}">
        <div class="sf-conn-dot"></div>
        ${SFStore.connected ? 'Salesforceに接続中' : 'Salesforce未接続（デモデータ表示中）'}
      </div>
      <h3>Salesforce接続設定</h3>
      <p>Salesforce組織に接続して、リアルタイムの営業データを取得します。<br>Connected App の設定情報を入力してください。</p>
      <div class="sf-form-group">
        <label class="sf-form-label">インスタンスURL</label>
        <input class="sf-form-input" id="sfInstanceUrl" type="url" placeholder="https://your-org.my.salesforce.com" value="${SFStore.config.instanceUrl}">
      </div>
      <div class="sf-form-group">
        <label class="sf-form-label">Client ID (Consumer Key)</label>
        <input class="sf-form-input" id="sfClientId" type="text" placeholder="3MVG9..." value="${SFStore.config.clientId}">
      </div>
      <div class="sf-form-group">
        <label class="sf-form-label">Client Secret (Consumer Secret)</label>
        <input class="sf-form-input" id="sfClientSecret" type="password" placeholder="Client Secretを入力" value="${SFStore.config.clientSecret}">
      </div>
      <div class="sf-form-group">
        <label class="sf-form-label">環境</label>
        <select class="sf-form-select" id="sfEnvironment">
          <option value="production">本番環境</option>
          <option value="sandbox">Sandbox</option>
        </select>
      </div>
      <div style="display:flex;gap:8px;margin-top:20px">
        <button class="sf-btn sf-btn-primary" id="sfConnectBtn">接続テスト</button>
        <button class="sf-btn sf-btn-outline" id="sfSaveBtn">設定を保存</button>
        ${SFStore.connected ? '<button class="sf-btn sf-btn-danger" id="sfDisconnectBtn">切断</button>' : ''}
      </div>
      <div id="sfConnMsg" style="margin-top:12px;font-size:13px"></div>
    </div>

    <div class="sf-panel" style="max-width:560px;margin:24px auto 0">
      <div class="sf-panel-header"><div class="sf-panel-title">データ同期設定</div></div>
      <div class="sf-panel-body">
        <div class="sf-form-group">
          <label class="sf-form-label">同期オブジェクト</label>
          <div style="display:flex;flex-direction:column;gap:6px;margin-top:4px">
            <label style="font-size:13px;display:flex;align-items:center;gap:6px"><input type="checkbox" checked disabled> Opportunity（商談）</label>
            <label style="font-size:13px;display:flex;align-items:center;gap:6px"><input type="checkbox" checked disabled> Account（取引先）</label>
            <label style="font-size:13px;display:flex;align-items:center;gap:6px"><input type="checkbox" checked> Task（活動）</label>
            <label style="font-size:13px;display:flex;align-items:center;gap:6px"><input type="checkbox" checked> Event（イベント）</label>
            <label style="font-size:13px;display:flex;align-items:center;gap:6px"><input type="checkbox"> Lead（リード）</label>
          </div>
        </div>
        <div class="sf-form-group">
          <label class="sf-form-label">自動同期間隔</label>
          <select class="sf-form-select">
            <option>手動のみ</option>
            <option>15分ごと</option>
            <option selected>1時間ごと</option>
            <option>6時間ごと</option>
            <option>1日1回</option>
          </select>
        </div>
  ...
  document.getElementById('sfConnectBtn').addEventListener('click', () => {
    const msg = document.getElementById('sfConnMsg');
    msg.innerHTML = '<span style="color:var(--sf-blue)">&#8987; 接続テスト中...</span>';
    setTimeout(() => {
      SFStore.config.instanceUrl = document.getElementById('sfInstanceUrl').value;
      SFStore.config.clientId = document.getElementById('sfClientId').value;
      SFStore.config.clientSecret = document.getElementById('sfClientSecret').value;

      if (SFStore.config.instanceUrl && SFStore.config.clientId) {
        SFStore.connected = true;
        SFStore.lastSync = new Date();
        msg.innerHTML = '<span style="color:var(--sf-green)">&#10003; 接続成功！Salesforceからデータを取得しました。</span>';
        updateSyncStatus();
        setTimeout(() => renderSettings(container), 1500);
      } else {
        msg.innerHTML = '<span style="color:var(--sf-red)">&#10007; 接続に失敗しました。インスタンスURLとClient IDを入力してください。</span>';
      }
    }, 1500);
  });
  ...
  document.getElementById('sfSaveBtn').addEventListener('click', () => {
    ...
    SFStore.config.clientId = document.getElementById('sfClientId').value;
    SFStore.config.clientSecret = document.getElementById('sfClientSecret').value;
    msg.innerHTML = '<span style="color:var(--sf-green)">&#10003; 設定を保存しました。</span>';
  });

  const disconnectBtn = document.getElementById('sfDisconnectBtn');
  if (disconnectBtn) {
    disconnectBtn.addEventListener('click', () => {
      SFStore.connected = false;
      SFStore.lastSync = null;
      updateSyncStatus();
